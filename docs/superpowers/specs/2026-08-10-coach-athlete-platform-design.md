...
| 날짜 | 동작 |
|---|---|
| **과거** (`date < today`) | 새 버전을 만들지 않는다. 기존 행을 **인플레이스 수정**한다(아래 참조). 새 버전 발행 시에는 이전 행을 그대로 승계 |
...
코치가 지나간 날짜의 오타나 잘못 올린 무게를 고쳐야 하는 경우가 있다. 이때는 행을 새로 만들지 않고 **기존 행의 내용만 제자리에서 수정한다.** 행 id가 유지되므로 선수의 로그와 저장된 요약이 깨지지 않는다.

대가는 선수가 당시 본 내용과 달라진다는 것이다. 이를 추적할 수 있도록 수정 이력을 `program_versions.note`에 append 한다(수정 일시·날짜·변경 요약).

### 불변식

이 규칙들이 보장하는 것은 다음 하나다.

> **과거 날짜 템플릿 행의 `id`는 절대 바뀌지 않는다.**

내용은 인플레이스로 바뀔 수 있으나 id는 고정이다. A.1의 행 승계가 안전한 것은 이 id 안정성 때문이지 내용 불변성 때문이 아니다.

### 누가 무엇을 언제 바꿀 수 있나

과거 잠금은 **발행된 프로그램 내용에만** 적용된다. 그것만이 남이 이미 본 것을 소급해서 바꾸는 행위이기 때문이다.

| 대상 | 코치 | 선수 |
|---|---|---|
| **프로그램 템플릿 (draft)** | 날짜 무관 자유 편집 | 볼 수 없음 |
| **프로그램 템플릿 (published)** | 과거: 인플레이스 수정 / 오늘: 확인 후 새 버전 / 미래: 새 버전 | 읽기 전용 |
| **개인 추가운동** | 관여 안 함 | 본인 것만, **날짜 무관** 추가·삭제 자유 |
| **로그 (기록·결과·메모)** | 읽기만, 그것도 공유된 날짜만 (C에서 구현) | 본인 것만, **날짜 무관** 자유 |

선수의 기록은 어떤 날짜든 언제든 고칠 수 있다. 지난주 목요일 결과를 오늘 뒤늦게 채워 넣는 동작은 현재도 되고 앞으로도 된다.

**변경 사항:** 현재 `duplicateSectionToToday`는 복제 대상 날짜를 오늘로 하드코딩하고 있다. 커스텀 로그(`CustomWorkoutForm`)가 이미 임의 날짜를 받는 것과 일관되도록, 이를 `duplicateSectionToDate(date, ...)`로 바꿔 임의 날짜를 허용한다. 지난주에 빠뜨린 보강운동을 뒤늦게 붙이는 경우를 위해서다.
...
POST   /api/workouts/duplicate     추가운동 복제. body에 대상 date 포함.
                                   owner_user_id = 세션 사용자 (클라이언트가 지정 불가)
